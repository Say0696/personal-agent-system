...
- Asks for explicit destination and scope before saving a reusable preference.
...
7. If a reusable correction appeared, the router asks whether to save it in the matching Skill's private overlay (default) or persistent `MEMORY.md` (only for important permanent cross-project information), then asks for scope.
...
Personal records are never part of the public tree. Ordinary lessons belong in a matching Skill-local overlay by default. Persistent `MEMORY.md` is reserved for explicitly approved important cross-project information. Use the local CLI for structured Skill-local records:
...
- A correction becomes a candidate only after the user chooses a destination and scope; silence never creates durable memory.
- Skill-local lessons are the default. Persistent `MEMORY.md` is an explicit exception.
